...
class MainWindow(QMainWindow):
    ...
    def _create_dashboard_tab(self):
        """Create enhanced dashboard tab with REAL-TIME P&L and data freshness indicators"""
        dashboard = QWidget()
        layout = QVBoxLayout(dashboard)

        # === LIVE DATA STATUS BAR (NEW - shows data freshness) ===
        status_bar = QHBoxLayout()
        
        # Live indicator (blinking dot)
        self.dash_live_indicator = QLabel("🟢 LIVE")
        self.dash_live_indicator.setStyleSheet("""
            QLabel {
                font-size: 14px;
                font-weight: bold;
                color: #4CAF50;
        ...
        """)
        status_bar.addWidget(self.dash_live_indicator)
        
        # Data freshness indicator
        self.dash_data_age = QLabel("Data: 0s ago")
        self.dash_data_age.setStyleSheet("""
            QLabel {
                font-size: 12px;
                color: #888;
                padding: 5px;
        ...
        """)
        status_bar.addWidget(self.dash_data_age)
        
        # Refresh rate indicator
        self.dash_refresh_rate = QLabel("⚡ Refresh: 2s")
        self.dash_refresh_rate.setStyleSheet("font-size: 12px; color: #2196F3; padding: 5px;")
        status_bar.addWidget(self.dash_refresh_rate)
        
        status_bar.addStretch()
        
        # Connection quality indicator
        self.dash_connection_quality = QLabel("📶 Good")
        self.dash_connection_quality.setStyleSheet("font-size: 12px; color: #4CAF50; padding: 5px;")
        status_bar.addWidget(self.dash_connection_quality)
        
        layout.addLayout(status_bar)

        # === Top Row - P&L Summary Cards ===
        pnl_group = QGroupBox("Today's Performance (Real-Time)")
        ...
        realized_card.setStyleSheet("QGroupBox { border: 1px solid #444; border-radius: 5px; }")
        realized_layout = QVBoxLayout(realized_card)
        self.dash_realized_pnl = QLabel("₹0.00")
        self.dash_realized_pnl.setStyleSheet("font-size: 28px; font-weight: bold; color: #4CAF50;")
        ...
        unrealized_card.setStyleSheet("QGroupBox { border: 1px solid #444; border-radius: 5px; }")
        unrealized_layout = QVBoxLayout(unrealized_card)
        self.dash_unrealized_pnl = QLabel("₹0.00")
        self.dash_unrealized_pnl.setStyleSheet("font-size: 28px; font-weight: bold; color: #2196F3;")
        ...
        self.dash_total_pnl.setStyleSheet("font-size: 32px; font-weight: bold;")
        ...
        trades_card.setStyleSheet("QGroupBox { border: 1px solid #444; border-radius: 5px; }")
        trades_layout = QVBoxLayout(trades_card)
        self.dash_trades_count = QLabel("0")
        self.dash_trades_count.setStyleSheet("font-size: 28px; font-weight: bold; color: #FF9800;")
        self.dash_trades_count.setAlignment(Qt.AlignmentFlag.AlignCenter)
        # Add trades today secondary label
        self.dash_trades_today = self.dash_trades_count  # Alias for compatibility
        trades_layout.addWidget(self.dash_trades_count)
        pnl_layout.addWidget(trades_card)

        # Win Rate Card
        winrate_card = QGroupBox("Win Rate")
        winrate_card.setStyleSheet("QGroupBox { border: 1px solid #444; border-radius: 5px; }")
        winrate_layout = QVBoxLayout(winrate_card)
        self.dash_win_rate = QLabel("0%")
        self.dash_win_rate.setStyleSheet("font-size: 28px; font-weight: bold; color: #9C27B0;")
        ...
        self.dash_available_margin.setStyleSheet("font-weight: bold; color: #4CAF50;")
        self.dash_used_margin = QLabel("₹0")
        self.dash_used_margin.setStyleSheet("font-weight: bold; color: #FF9800;")
        self.dash_total_balance = QLabel("₹0")
        self.dash_total_balance.setStyleSheet("font-weight: bold; color: #2196F3;")
        self.dash_broker_status = QLabel("Not Connected")
        self.dash_broker_status.setStyleSheet("color: red; font-weight: bold;")
        ...
        # Market Status (Enhanced with live prices)
        market_group = QGroupBox("Market Status (Live)")
        market_layout = QFormLayout(market_group)
        self.dash_market_status = QLabel("Closed")
        self.dash_market_status.setStyleSheet("color: red; font-weight: bold; font-size: 14px;")
        self.dash_nifty_price = QLabel("--")
        self.dash_nifty_price.setStyleSheet("font-weight: bold; font-size: 14px;")
        self.dash_nifty_change = QLabel("")  # NEW - price change indicator
        self.dash_banknifty_price = QLabel("--")
        self.dash_banknifty_price.setStyleSheet("font-weight: bold; font-size: 14px;")
        self.dash_banknifty_change = QLabel("")  # NEW - price change indicator
        self.dash_last_update = QLabel("--")
        self.dash_last_update.setStyleSheet("color: #888;")
        market_layout.addRow("Market:", self.dash_market_status)
        
        # NIFTY with change
        nifty_row = QHBoxLayout()
        nifty_row.addWidget(self.dash_nifty_price)
        nifty_row.addWidget(self.dash_nifty_change)
        nifty_widget = QWidget()
        nifty_widget.setLayout(nifty_row)
        market_layout.addRow("NIFTY:", nifty_widget)
        
        # BANKNIFTY with change
        banknifty_row = QHBoxLayout()
        banknifty_row.addWidget(self.dash_banknifty_price)
        banknifty_row.addWidget(self.dash_banknifty_change)
        banknifty_widget = QWidget()
        banknifty_widget.setLayout(banknifty_row)
        market_layout.addRow("BANKNIFTY:", banknifty_widget)
        ...
        self.dash_open_positions.setStyleSheet("font-weight: bold;")
        ...
        positions_group = QGroupBox("Open Positions (Real-Time P&L)")
        ...
        # Auto-refresh toggle
        self.dash_auto_refresh_check = QCheckBox("Auto-refresh (2s)")
        self.dash_auto_refresh_check.setChecked(True)
        self.dash_auto_refresh_check.stateChanged.connect(self._toggle_auto_refresh)
        refresh_row.addWidget(self.dash_auto_refresh_check)
        ...
        # Set up auto-refresh timer for dashboard - FAST REFRESH
        self.dashboard_timer = QTimer(self)
        self.dashboard_timer.timeout.connect(self._refresh_dashboard)
        self.dashboard_timer.start(2000)  # Refresh every 2 seconds for real-time feel
        
        # Store previous prices for change calculation
        self._prev_nifty_price = 0
        self._prev_banknifty_price = 0
        
    def _toggle_auto_refresh(self, state):
        """Toggle auto-refresh for dashboard"""
        if state == Qt.CheckState.Checked.value:
            self.dashboard_timer.start(2000)
            self.dash_live_indicator.setText("🟢 LIVE")
            self.dash_live_indicator.setStyleSheet("""
                QLabel {
            ...
            self.dash_live_indicator.setText("⏸️ PAUSED")
            self.dash_live_indicator.setStyleSheet("""
                QLabel {
                    font-size: 14px; font-weight: bold; color: #FF9800;
                    padding: 5px 10px; background: #1E1E1E;
                    border-radius: 5px; border: 1px solid #FF9800;
                }
            """)
...
/help - Show all commands""")
    ...
    def _setup_timers(self):
        """Setup refresh timers - OPTIMIZED FOR LOW LATENCY"""
        # Fast refresh for critical data (every 1 second for live trading)
        self.refresh_timer = QTimer(self)
        self.refresh_timer.timeout.connect(self._refresh_data)
        self.refresh_timer.start(3000)  # Changed from 30s to 3s

        # Ultra-fast refresh for live prices (every 1 second)
        self.ltp_timer = QTimer(self)
        self.ltp_timer.timeout.connect(self._refresh_live_prices)
        self.ltp_timer.start(1000)  # Changed from 5s to 1s for real-time feel
        
        # Track last update times for data freshness indicator
        self.last_ltp_update = datetime.now()
    ...
    def _refresh_dashboard(self):
        """Refresh dashboard with live data - OPTIMIZED FOR REAL-TIME"""
        try:
            from datetime import datetime
            
            # Update data freshness indicator
            if hasattr(self, 'last_data_update'):
                age_seconds = (datetime.now() - self.last_data_update).total_seconds()
                if hasattr(self, 'dash_data_age'):
                    self.dash_data_age.setText(f"Data: {int(age_seconds)}s ago")
                    # Color code based on freshness
                    if age_seconds < 3:
                        self.dash_data_age.setStyleSheet("font-size: 12px; color: #4CAF50; padding: 5px;")
                    elif age_seconds < 10:
                        self.dash_data_age.setStyleSheet("font-size: 12px; color: #FF9800; padding: 5px;")
                    else:
                        self.dash_data_age.setStyleSheet("font-size: 12px; color: #F44336; padding: 5px;")
    ...
    def _update_dashboard_positions_combined(self, positions: list):
        """Update dashboard with both broker and paper positions - ENHANCED WITH CHANGE INDICATOR"""
        self.dash_positions_table.setRowCount(len(positions))
        self.dash_open_positions.setText(str(len(positions)))
        ...
        for i, pos in enumerate(positions):
            ...
            price_change = ltp - avg_price if avg_price > 0 else 0
            ...
            # NEW: Change column with color
            change_item = QTableWidgetItem(f"{price_change:+.2f} ({price_change_pct:+.1f}%)")
            change_item.setForeground(Qt.GlobalColor.green if price_change >= 0 else Qt.GlobalColor.red)
            self.dash_positions_table.setItem(i, 5, change_item)

            pnl_item = QTableWidgetItem(f"₹{pnl:+,.2f}")
            pnl_item.setForeground(Qt.GlobalColor.green if pnl >= 0 else Qt.GlobalColor.red)
            self.dash_positions_table.setItem(i, 6, pnl_item)
            ...
            self.dash_positions_table.setItem(i, 7, pnl_pct_item)
            ...
            self.dash_positions_table.setItem(i, 8, source_item)
            ...
            if is_paper:
                ...
                close_btn = QPushButton("Close")
                close_btn.clicked.connect(lambda checked, s=symbol, q=qty: self._close_position(s, q))
            self.dash_positions_table.setCellWidget(i, 9, close_btn)

    def _update_dashboard_positions_paper(self, positions: list):
        """Update dashboard positions table with paper trading positions - ENHANCED"""
        self.dash_positions_table.setRowCount(len(positions))
        self.dash_open_positions.setText(str(len(positions)))
        ...
        for i, pos in enumerate(positions):
            ...
            source_item = QTableWidgetItem("PAPER")
            source_item.setForeground(Qt.GlobalColor.yellow)
            self.dash_positions_table.setItem(i, 8, source_item)
